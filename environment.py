...
class Environment(testmod.Environment):
    ...
    def init(self, name):
        ...
        c = self.c1.new_client("Linux NFSv4.1 %s" %str(name+str(random.randint(0, 100))))
        sess = c.create_session()
        res = sess.compound([op.reclaim_complete(FALSE)])
        res = sess.compound([op.putrootfh(), op.secinfo_no_name(0)])
        res = sess.compound([op.putrootfh(), op.getfh(), op.getattr(nfs4lib.list2bitmap([1,3,4,8,20,33,35,36,37,41,45,47,52,53,55]))])
        fh = res.resarray[1].object
        print("-1-----------------")
        print (fh)
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([0, 2, 5, 6, 13, 75]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([10, 27, 30, 31, 51, 62 ,65]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([0, 2, 5, 6, 13, 75]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([10, 27, 30, 31, 51, 62, 65]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([28, 29]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([0, 2, 5, 6, 13, 75]))])
        res = sess.compound([op.putfh(fh), op.getattr(nfs4lib.list2bitmap([1,3,4,8,20,33,35,36,37,41,45,47,52,53,55]))])
        res = sess.compound([op.putfh(fh), op.access(0x1f), op.getattr(nfs4lib.list2bitmap([3, 4, 52, 53]))])
        res = sess.compound([op.putfh(fh), op.lookup("nfs"), op.getfh(), op.getattr(nfs4lib.list2bitmap([1,3,4,8,20,33,35,36,37,41,45,47,52,53,55]))])
